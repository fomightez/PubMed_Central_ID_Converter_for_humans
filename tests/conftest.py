...
I needed `uv pip install -e .` at the start of test phase because the CLI entry point wassn't registered yet.
...
KEEP IN MIND YOU CAN JUST RUN `uv pip install -e .` AT ANY POINT TO INSTALL IN
EDITABLE MODE TO TEST LATEST VERSION OF CODE IN THE SESSION. The `use_freshest_in_session_package_code` just applies a nice handle on it to do
more easiky when dealing with the tests.
'''
